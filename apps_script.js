...
function doPost(e) {
  ...
  // ── PRIORIDADES — agregar ────────────────────────────────
  if (accion === "agregar_prioridad") {
    var hoja = ss.getSheetByName("PRIORIDADES");
    if (!hoja) {
      hoja = ss.insertSheet("PRIORIDADES");
      hoja.appendRow(["ID", "Campo", "Valor", "Activo", "Creado"]);
    }
    hoja.appendRow([body.id, body.campo, body.valor, "si", body.creado]);
    return json({ ok: true });
  }

  // ── PRIORIDADES — borrar ─────────────────────────────────
  if (accion === "borrar_prioridad") {
    var hoja = ss.getSheetByName("PRIORIDADES");
    if (!hoja) return json({ ok: false, error: "Hoja no existe" });
    var rows = hoja.getDataRange().getValues();
    for (var i = rows.length - 1; i >= 1; i--) {
  ...
  // ── PRIORIDADES — toggle ─────────────────────────────────
  if (accion === "toggle_prioridad") {
    var hoja = ss.getSheetByName("PRIORIDADES");
    if (!hoja) return json({ ok: false, error: "Hoja no existe" });
    var rows = hoja.getDataRange().getValues();
    for (var i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === String(body.id)) {
        hoja.getRange(i + 1, 4).setValue(body.activo);
        return json({ ok: true });
      }
    }
    return json({ ok: false, error: "Regla no encontrada" });
  }
